    ...
    if(l->head == NULL){
      ...
      if(current->next == NULL) {  //only one node in list
         if(newblk->start <= current->blk->start) {   
            newNode->next = l->head; // insert newblk after current
            l->head = newNode;
         }
         else {   // Insert newblk After Current Node if:   newblk->start > c->start
            current->next = newNode; //empty list case
            newNode->next = NULL;
         }
      }
      else {  // two or more nodes in list

         if(newNode->blk->start <= current->blk->start) {  // insert before current node
            newNode->next = l->head;
            l->head = newNode;
         }
         else {
            ...
            prev->next = newNode;
            newNode->next = current; //continue traversing
         }
      }
    }
}
