...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
   ...
   //create node for the block
    ...
    node_t *temp = freelist->head;
    node_t *prev = NULL; 
    
    while(temp) { // go thtough freelist until end
      if (temp->blk->end - temp->blk->start >= blocksize){ // Check if a node is in the FREE_LIST with a blk(end - start) >= blocksize
         block = temp; //remove it and go to #3, 
        break;
      } 
      prev = temp;
      temp = temp->next;
    ...
    if (! block){ 
      printf("Error: Memory Allocation %d blocks\n", blocksize); // if not in FREE LIST print ""Error: Memory Allocation <blocksize> blocks\n""
      return;
    }
    else {   // if block is in there, 
      if (!prev){ 
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
    node_t *temp = alloclist->head;
    node_t *prev = NULL;
    block_t * deallocate_block = NULL;

    while (temp && temp->blk->pid != pid) { // go through alloc list
      prev = temp;
      temp = temp->next;
    }
  
    if (!temp){ //    * 1. Check if a node is in the ALLOC_LIST with a blk.pid = pid
